...
{
    public class PersonalisationProvider<T> : IPersonalisationProvider<T> where T : TreeNode, new()
    {
        ...
        public List<PersonalizedContent<T>> ContentList { get; set; }
        public List<T> GetPersonalizedItems()
        {
            ...
            if (CurrentPersona == null)
            {
                ...
                return ContentList.Select(item => item.Item).ToList();
            }
            ...
            var result = ContentList.OrderBy(item => item.Relevance).Select(item => item.Item).ToList();
        ...
        {
            ContentList = ContentHelper.GetDocs<T>(_className).Select(item => new PersonalizedContent<T>(item)).ToList();
        }
